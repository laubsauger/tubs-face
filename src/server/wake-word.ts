...
export function detectWakeWord(text: string): WakeWordResult {
  ...
  const weak = isWeakWakeAlias(wakeMatch.token);
  ...
  return {
    ...
    normalized,
    version: WAKE_MATCHER_VERSION,
    ...(hasWakeToken && wakeMatch.token ? { matchedToken: wakeMatch.token } : {}),
    ...(wakeMatch.source ? { matchedSource: wakeMatch.source } : {}),
  };
}
...
function findWakeToken(tokens: string[]): WakeTokenMatch {
  const directIndex = tokens.findIndex((token) => isWakeAlias(token));
  if (directIndex !== -1) {
    return { index: directIndex, token: tokens[directIndex] ?? null, source: 'token' };
  }
  ...
  if (tokens.length <= 3) {
    ...
    for (let index = 0; index < tokens.length - 1; index += 1) {
      ...
      if (isWakeAlias(merged)) {
        return { index, token: merged, source: 'merged' };
      }
    }
  }
...
function isWakeAlias(token: string): boolean {
  if (!token || token.length < 3 || token.length > 8) {
    return false;
  }

  for (const candidate of getWakeCandidates(token)) {
    if (WAKE_ALIASES.has(candidate) || WAKE_WEAK_ALIASES.has(candidate)) {
      return true;
    }
    if (candidate.length < 3 || candidate.length > 6) {
      continue;
    }
    if (levenshteinDistance(candidate, 'tubs') <= 1) {
      return true;
    }
    if (levenshteinDistance(candidate, 'tub') <= 1) {
      return true;
    }
...
function isWeakWakeAlias(token: string | null): boolean {
  if (!token || token.length < 3 || token.length > 8) {
    return false;
  }
  return Array.from(getWakeCandidates(token)).some((candidate) => WAKE_WEAK_ALIASES.has(candidate));
}

function getWakeCandidates(token: string): Set<string> {
